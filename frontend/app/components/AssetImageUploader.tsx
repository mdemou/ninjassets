...
export function AssetImageUploader({ assetId, hasImage, onChanged }: AssetImageUploaderProps) {
  const { t } = useLanguage();
  ...
  const handleFile = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      await api.upload(`/api/p/assets/${assetId}/image`, file);
  ...
  const handleRemove = async () => {
    setRemoving(true);
    try {
      await api.delete(`/api/p/assets/${assetId}/image`);
