...
- **Import/export worker** — `importExportWorker.ts` is event-driven (BLPOP on `config.db.redis.queues.importExportJobs`); the DB drain is authoritative. A scheduler sweep (`IMPORT_SAFETY_SWEEP_MS`) catches jobs enqueued while Redis was down. Disabled with `IMPORT_EXPORT_ENABLED=false` when running a separate worker process.
...
Async jobs (dry-run, commit, export) are drained by `infrastructure/proceses/importExportWorker.ts`. The worker blocks on `config.db.redis.queues.importExportJobs` (`IMPORT_WORKER_BLOCK_SECONDS`) and fully drains pending jobs from the DB on each wakeup; the scheduler's `import-export-sweep` (`IMPORT_SAFETY_SWEEP_MS`) is a safety net when Redis was down. In dev the worker runs in-process (started from `proceses/init.ts`); in production run it as a **separate worker process** (D-IMPORT-3) and set `IMPORT_EXPORT_ENABLED=false` on API nodes. Uploaded files and artifacts live under `IMPORT_STORAGE_PATH` and are purged after `IMPORT_ARTIFACT_RETENTION_DAYS` (also via scheduler `import-artifact-purge`). See [spec-import-export.md](../docs/spec-import-export.md).
